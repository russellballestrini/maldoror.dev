...
const VERSION = process.env.MALDOROR_LANDMARK_VERSION ?? 'v2';
if (!/^v\d+$/.test(VERSION)) throw new Error(`Invalid landmark version: ${VERSION}`);
const SOURCE = path.join(ROOT, `assets/biomes/generated/regional-landmark-atlas-${VERSION}-source.png`);
const OUTPUT = path.join(ROOT, 'assets/biomes/landmarks');
...
const ROWS = 2;
const NAMES = [
  'canal-town-waterfront-cluster',
  'forest-canopy-cluster',
  'coast-marsh-beacon-cluster',
  'rural-orchard-farmstead-cluster',
  'mountain-cave-outcrop-cluster',
  'ruins-overgrown-arch-cluster',
];

if (!fs.existsSync(SOURCE)) throw new Error(`Regional landmark source is missing: ${SOURCE}`);
...
if (width < 300 || height < 200 || width % COLUMNS !== 0 || height % ROWS !== 0) {
  throw new Error(`Expected a regular ${COLUMNS}x${ROWS} atlas; received ${width}x${height}`);
}

fs.mkdirSync(OUTPUT, { recursive: true });
const temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'maldoror-landmarks-'));
const cellWidth = width / COLUMNS;
const cellHeight = height / ROWS;
const inset = Math.max(6, Math.round(Math.min(cellWidth, cellHeight) * 0.014));
const derived = [];
...
try {
  for (let index = 0; index < NAMES.length; index++) {
    ...
    if (coverage < 0.12 || coverage > 0.82 || cornerOffsets.some((offset) => data[offset] !== 0)) {
      throw new Error(`Invalid alpha extraction for ${NAMES[index]}: coverage=${coverage.toFixed(4)}`);
    }
    derived.push({
      id: NAMES[index],
      file: path.relative(path.dirname(path.join(ROOT, 'assets/biomes/landmarks-manifest.json')), outputPath),
