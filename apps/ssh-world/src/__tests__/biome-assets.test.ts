...
describe('regional biome material manifest', () => {
  ...
  it('loads explicit modular parcel masses for every biome family', async () => {
    ...
    expect(new Set(ruralFocals.map((asset) => asset.frontageAxis)))
      .toEqual(new Set(['east-west', 'north-south']));
    expect(new Set(ruralFocals.map((asset) => asset.compositionSide)))
      .toEqual(new Set([-1, 1]));
    expect(new Set(ruralFocals.map((asset) => asset.visualGroup)))
